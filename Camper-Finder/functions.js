...
class Vector {
    ...
    static calcHypo(adjacent, opposite) {
...
function windowOnClickManu(event) {
...
function windowOnClickPrice(event) {
...
function windowOnClickColour(event) {
...
function containsAllLetters(a, b) {
    let matches = 0;
...
function filterByPrice() {
    ...
    filteredVans = vans.filter((v) => v.price <= parseInt($('what-Price').value));
    vans = filteredVans;
}
...
function countFeatures(feature) {
    let count = 0;
    for (let i = 0; i < vans.length; i++) {
...
function domFeatureCheckboxes() {
    ...
    for (let k in features) {
        ...
        featureInput.name = features[k];
        featureInput.value = k;
        featureInput.classList.add('featuresCheckbox');
        featureInput.id = 'whichFeature';
        featureInput.addEventListener("click", (e) => filterByFeature(k, e.target.checked));
        let div = document.createElement("div");
        let label = document.createElement('label');
